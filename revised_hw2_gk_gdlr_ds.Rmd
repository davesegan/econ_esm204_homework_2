...
# Supply curves (marginal abatement costs)
...
# Demand curves (marginal benefit of each additional ton of pollution)
# q = new emissions level
a_price <- function(q) {a_coeff*(a_emissions_current - q) + a_const}
b_price <- function(q) {b_coeff*(b_emissions_current - q) + b_const}
c_price <- function(q) {c_coeff*(c_emissions_current - q) + c_const}
d_price <- function(q) {d_coeff*(d_emissions_current - q) + d_const}
...
```{r sector B MC Curve, include = FALSE}
# ----------------
# Sector B - MC curve
# ----------------
sector_b <- mc_sector %>% 
...
# predict it back to the original data frame
predict_df_b <- mc_sector %>% 
  mutate(pred_b = predict(lm_sector_b, mc_sector))

# data frame with both actual mc and predicted 
df_b <- data.frame(sector_b, predict_df_b)
...
c_demand <- function(q) {c_coeff*(c_emissions_current - q) + c_const}
...
d_supply <- function(abatement) {d_coeff*abatement + d_const}
d_price <- function(q) {
  d_coeff*(d_emissions_current-q) + d_const
}
q_pollution <- seq(0,300, by = 10)
